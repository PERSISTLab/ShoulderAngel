...
const uint8_t recoveryPacket[RECOVERY_PACKET_SIZE] = {
    ...
    uint8_t count = 0;
    uint8_t triggered = NO;
    uint8_t j;
    while(1) {
        receive_single_packet_over_radio(rxData, &rxSize);
        if (compare_packets(rxData, rxSize, packet1, PACKET_SIZE_1) == CORRECT) {
            LED0_TOGGLE();
            for (j = 0; j < 10; j++) {
                __delay_cycles(1000);
                LED0_TOGGLE();
        ...
        } else if (compare_packets(rxData, rxSize, packet3, PACKET_SIZE_3) == CORRECT) {
            LED1_TOGGLE();
            triggered = YES;
            for (j = 0; j < 10; j++) {
                __delay_cycles(1000);
        ...
        } else if (compare_packets(rxData, rxSize, packet2, PACKET_SIZE_2) == CORRECT) {
            if (triggered == YES) {
                for (j = 0; j < 10; j++) {
                    __delay_cycles(1000);
                    LED0_TOGGLE();
                    LED1_TOGGLE();
                    send_single_packet_over_radio(recoveryPacket, RECOVERY_PACKET_SIZE);
                    triggered = NO;
                }
            } else {
                for (j = 0; j < 10; j++) {
